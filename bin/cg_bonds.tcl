...
proc parse_gmxdump_output { molid gmxdump tpr topology_type BD ELAST CNSTR } {
  ...
  puts "going to dump tpr "
   set tpr [open "| $gmxdump dump -s $tpr 2> /dev/null | grep -F -e \#atoms -e \#beads -e moltype -e \#molecules -e \(BONDS\) -e \(CONSTR\) -e \(HARMONIC\) -e \(SETTLE\)" "r"]
  ...
  while { [gets $tpr line] > 0 } {
    ...
    # rpb says no reason to spam the terminal: puts "got another line "
    ...
    # ryan bradley ("rpb") adds this so we pick up SETTLE bonds in water and hence can use the script 
    # ...to ensure that long bonds in other molecules (e.g. PtdIns) is rendered correctly
    if { [regexp {\(SETTLE\)\s+(\d+)\s+(\d+)\s+(\d+)} $line 0 first_bead second_bead third_bead] } {
      add_link $molecule_id $first_bead $second_bead bonds
      add_link $molecule_id $first_bead $third_bead bonds
    }
  ...
  # rpb says no reason to spam the terminal: puts "gmxdump done "
  }
